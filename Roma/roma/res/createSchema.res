[de]
helpmessage=Sie können wählen, ob das Schema in Relax NG, W3C, oder DTD Syntax erstellt werden soll.
headline=Zeit für das Schema
form_headline=Schema machen
form_output=Welches Format bevorzugen Sie?
schemaHelp=We use RELAX NG but for help see: 


[en]
helpmessage=You can choose whether to make a schema in Relax, W3C or DTD
headline=Time to give you a schema
form_headline=Creating a schema
form_output=Which format do you prefer?
schemaHelp=We use RELAX NG but for help see: 


[es]
helpmessage=Puede elegir entre construir el esquema en Relax, en W3C o en DTD
headline= Momento de crear el esquema 
form_headline= Crea tu esquema
form_output=¿Qué formato prefieres?
schemaHelp=We use RELAX NG but for help see: 

[fr]
helpmessage=On y va! création du schéma
headline=Enfin je vous rends votre schéma...
form_headline=Creation du schéma en cours
form_output=Quel format préfériez-vous?
schemaHelp=We use RELAX NG but for help see: 


[it]
helpmessage=Puoi scegliere se costruire lo schema XML in Relax, W3C o DTD
headline= Creazione dello schema 
form_headline= Crea il tuo schema
form_output=Quale formato preferisci?
schemaHelp=We use RELAX NG but for help see: 


[ja]
helpmessage=スキーマを選択し、自動作成します。
headline=スキーマの作成
form_headline=スキーマの作成
form_output=スキーマの選択
schemaHelp=We use RELAX NG but for help see: 


[ru]
helpmessage=Вы можете по своему усмотрению создать схему в Relax, W3C или в DTD 
headline=Время выбрать схему
form_headline=Создание схемы
form_output=Какой формат Вы предпочитаете?
schemaHelp=We use RELAX NG but for help see: 


[sw]
helpmessage=Du kan välja att göra ett schema i formaten Relax, W3C or DTD
headline=Dags att ge dig ett schema
form_headline=Skapar ett schema
form_output=Vilket format föredrar du?
schemaHelp=We use RELAX NG but for help see: 


[zh-TW]
helpmessage=您可以選擇Relax NG、W3C或DTD的模型語言來建立文件模型檔。
headline=建立文件模型
form_headline=建立文件模型
form_output=您想要哪一種模型語言?
schemaHelp=We use RELAX NG but for help see: 


[pt]
helpmessage=Pode criar um esquema em Relax, W3C ou DTD
headline=Criaçao de um esquema
form_headline=Criaçao de um esquema
form_output=Que formato prefere?
schemaHelp=We use RELAX NG but for help see: 


