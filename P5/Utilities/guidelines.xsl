<xsl:stylesheet
  exclude-result-prefixes="xlink dbk rng tei teix xhtml a html pantor xd xs xsl fo"
  version="2.0"
  ...
  <xsl:import
      href="/usr/share/xml/tei/stylesheet/odds2/odd2html.xsl"/>
  ...
  <xsl:key name="HEADS" match="tei:head"
	   use="concat(@xml:lang,@corresp)"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="key('ELEMENTDOCS',1)">
      ...
      <xsl:variable name="mychapter" select="ancestor::tei:div[last()]/@xml:id"/>
	...
	<html>
	  ...
	  <body id="TOP">
		...
		<xsl:call-template name="makeGloss">
	      ...
	      their encoding.</p>	      
		<xsl:for-each
		    select="/tei:TEI/tei:text/tei:body/tei:div[@xml:id=$mychapter]">
		...
		<xsl:for-each
		    select="/tei:TEI/tei:text/tei:body/tei:div[not(@xml:id=$mychapter)]">
		  <xsl:if test="count(key('EXAMPLES',concat(@xml:id,$me)))&gt;0">
		    <li>
		      <a href="#{@xml:id}">
			<xsl:call-template name="header"/>
			<xsl:if test="./ancestor::tei:*/@module = $me/ancestor::tei:*/@module">
	...
	<p>
	  <xsl:choose>
	    <xsl:when test="ancestor::tei:elementSpec">
	      <a
		  href="../../{ancestor::tei:exemplum/@xml:lang}/html/ref-{ancestor::tei:elementSpec/@ident}.html">
		<xsl:text>&lt;</xsl:text>
		<xsl:value-of
	    ...
	    <xsl:when test="ancestor::tei:classSpec">
	      <a
		  href="../../{ancestor::tei:exemplum/@xml:lang}/html/ref-{ancestor::tei:classSpec/@ident}.html">
		<xsl:value-of
	...
	<xsl:value-of select="count(ancestor::tei:div)"/>
	  ...
	  <xsl:choose>
		...
		<xsl:text>tocTree</xsl:text>
	      ...
	      <span class="showhide">
		<span class="tocShow"><img alt="+" src="icons/plus.gif"/> </span>
		<span class="tocHide"><img alt="-" src="icons/minus.gif"/> </span>
	      </span> 
	    </xsl:when>
	    <xsl:otherwise>
	      <xsl:attribute name="class">
...
<xsl:template match="tei:listBibl">
  <xsl:choose>
    <xsl:when test="@xml:lang and not($documentationLanguage=@xml:lang)">
    </xsl:when>
    <xsl:otherwise>
      <xsl:apply-imports/>
    </xsl:otherwise>
...
<xsl:template name="egXMLEndHook">
  <xsl:if test="@corresp and key('IDS',substring(@corresp,2))">
    <div style="float: right;">
	<a>
	  <xsl:attribute name="href">
	    <xsl:apply-templates mode="generateLink"
				 select="key('IDS',substring(@corresp,2))"/>
	  </xsl:attribute>
...
<xsl:template name="figureHook">
  ...
  <xsl:template name="stdfooter">
	...
	<div class="availability">
      ...
      <address>
	<br/>
        <xsl:text>TEI Guidelines Version </xsl:text>
        <xsl:value-of 
...
<!-- German -->
  ...
  <head xml:lang="ko" corresp="AI">단순 분석 기제</head>
  <head xml:lang="ko" corresp="CE">확실성과 불확실성</head>
  <head xml:lang="ko" corresp="CO">모든 TEI 문서에 공통된 요소들</head>
  <head xml:lang="ko" corresp="CC">코퍼스 텍스트</head>
  <head xml:lang="ko" corresp="DI">인쇄 사전</head>
  <head xml:lang="ko" corresp="DR">공연 텍스트</head>
  <head xml:lang="ko" corresp="FT">표, 식, 그림</head>
  <head xml:lang="ko" corresp="WD">문자와 그림문자 문서</head>
  <head xml:lang="ko" corresp="HD">TEI 헤더</head>
  <head xml:lang="ko" corresp="FS">자질 구조</head>
  <head xml:lang="ko" corresp="SA">연결, 분할, 정렬</head>
  <head xml:lang="ko" corresp="MS">필사본 기술</head>
  <head xml:lang="ko" corresp="ND">이름과 날짜</head>
  <head xml:lang="ko" corresp="GD">그래프, 망, 수형도</head>
  <head xml:lang="ko" corresp="TS">전사된 대화(말)</head>
  <head xml:lang="ko" corresp="TD">TEI 모듈 문서</head>
  <head xml:lang="ko" corresp="ST">모든 TEI 모듈에 사용되는 부류, 자료유형, 마크로를 위한 선언</head>
  <head xml:lang="ko" corresp="TC">비평적 주석</head>
  <head xml:lang="ko" corresp="DS">기본값 텍스트 구조</head>
  <head xml:lang="ko" corresp="PH">일차 원전 전사</head>
  <head xml:lang="ko" corresp="VE">운문 구조</head>
  <head xml:lang="ko" corresp="COL">판권 면</head>
  <head xml:lang="ko" corresp="dedication">헌정</head>
  <head xml:lang="ko" corresp="PREF">서문과 사사 </head>
  <head xml:lang="ko" corresp="PREFS">서문</head>
  <head xml:lang="ko" corresp="REF-CLASSES-ATTS">속성 부류</head>
  <head xml:lang="ko" corresp="REF-CLASSES-MODEL">모형 부류</head>
  <head xml:lang="ko" corresp="REF-ELEMENTS">요소</head>
  <head xml:lang="ko" corresp="REF-MACROS">자료유형과 다른 마크로</head>
  ...
  <xsl:template name="calculateFigureNumber">
	...
	<xsl:text> </xsl:text>
	<xsl:number count="tei:figure[tei:head]" from="tei:div[parent::tei:front]" level="any"/>
	<xsl:for-each select="ancestor::tei:div[parent::tei:front]">
	  <xsl:number/>
	</xsl:for-each>
	<xsl:text>.</xsl:text>
      </xsl:when>
	...
	<xsl:text> </xsl:text>
	<xsl:for-each select="ancestor::tei:div[parent::tei:back]">
	  <xsl:number/>
	</xsl:for-each>
	<xsl:text>.</xsl:text>
	<xsl:number count="tei:figure[tei:head]" from="tei:div[parent::tei:back]" level="any"/>
      </xsl:when>
	...
	<xsl:text> </xsl:text>
	<xsl:for-each select="ancestor::tei:div[parent::tei:body]">
	  <xsl:number/>
	</xsl:for-each>
	<xsl:text>.</xsl:text>
	<xsl:number count="tei:figure[tei:head]" from="tei:div[parent::tei:body]" level="any"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
