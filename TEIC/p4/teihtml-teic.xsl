...
-->
 <xsl:if test="/TEI.2/teiHeader/fileDesc/notesStmt/note[@type='rsslink']/xref">
   <xsl:for-each select="/TEI.2/teiHeader/fileDesc/notesStmt/note[@type='rsslink']">
     <link href="{xref/@url}" rel="alternate" type="application/rss+xml" title="{xref}" />
   </xsl:for-each>
 </xsl:if>
...
<xsl:template name="stdfooter">
 ...
 <address>
   ...
   <xsl:choose>
     <xsl:when
	 test="/TEI.2/teiHeader/fileDesc/publicationStmt/availability/rdf:RDF/cc:License">
       This document is copyright <xsl:call-template name="generateAuthor"/>
       and licensed under a <a>
       <xsl:attribute name="href">
	 <xsl:value-of
	     select="/TEI.2/teiHeader/fileDesc/publicationStmt/availability/rdf:RDF/cc:License/@rdf:about"/>
       </xsl:attribute>
...
<!--
  <p align="right">
   <xsl:for-each select="document('/Applications/preface.xml')/TEI.2/text/body">
    <xsl:apply-templates/> 
   </xsl:for-each>
  </p>
</xsl:if>
...
<xsl:value-of select="count(TEI.2)"/> projects represented here.
...
<tr>
 ...
 <td valign="top">
   ...
   <ul>
    <xsl:for-each select="//TEI.2">
    <xsl:sort select=".//resp[text()!='edit'][1]/ancestor::change/date/@value" order="descending" data-type="text"/>
     <li>
      <a href="{@id}.xml">
	<xsl:value-of select=".//titleStmt/title"/>
      </a><xsl:text> </xsl:text>
...
<xsl:template name="latestUpdate">
  <xsl:variable name="respCreate" select="teiHeader//resp[text()='create']"/>
  <xsl:variable name="respUpdate" select="teiHeader//resp[text()='update']"/>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:value-of select="teiHeader//resp[text()='create']/ancestor::change/date[1]"/>
    </xsl:otherwise>
  </xsl:choose>
  <xsl:if test="$respUpdate">
    <xsl:text>; last update </xsl:text>
    <xsl:for-each select="$respUpdate">
      <xsl:sort select="ancestor::change/date/@value" order="descending" data-type="text"/>
      <xsl:if test="position()=1">
        <xsl:value-of select="ancestor::change/date[1]"/>
      </xsl:if>
    </xsl:for-each>
  </xsl:if>
</xsl:template>
...
<xsl:template match="xptr[@type='transclude' and
