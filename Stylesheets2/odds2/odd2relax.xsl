...
<xsl:stylesheet exclude-result-prefixes="t tei a rng s xd xlink xs teix" version="2.0"
    ...
    <xsl:if test="$verbose='true'">
      <xsl:message> I18N setup: Pattern prefix: <xsl:value-of select="$patternPrefixText"/> Target
        language: <xsl:value-of select="$targetLanguage"/> Documentation language: <xsl:value-of
          select="$documentationLanguage"/> Parameterization: <xsl:value-of select="$parameterize"/>
      </xsl:message>
    </xsl:if>
