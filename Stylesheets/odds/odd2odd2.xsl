...
<xsl:stylesheet version="2.0" xmlns:s="http://www.ascc.net/xml/schematron"
  ...
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="teix a s tei rng xd">
  ...
  <xsl:key name="MEMBEROFDELETE" match="tei:memberOf[@mode='delete']"
    use="concat(../../@ident,@key)"/>
  <xsl:key name="MEMBEROFADD" match="tei:memberOf[not(@mode='delete')]"
    use="concat(../../@ident,@key)"/>
  ...
  <xsl:key match="tei:classSpec[(tei:attList or @type='atts') and not(@ident='tei.TEIform')]"
...
If so, use them as is. 
 -->
  ...
  <xsl:template match="rng:choice|rng:list|rng:group|rng:optional|rng:oneOrMore|rng:zeroOrMore"
    mode="copy">
    <xsl:call-template name="simplifyRelax"/>
  </xsl:template>
...
-->
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="$element='oneOrMore' and $entCount=1 and
		      local-name($contents/WHAT/*)='zeroOrMore'">
  ...
  <xsl:template name="changedAtt">
    ...
    <xsl:choose>
	 ...
	 if we already have an attRef -->
      ...
      <xsl:when test="key('DELETEATT',$lookingAt)"/>
      <xsl:when test="key('REPLACEATT',$lookingAt)">
        <xsl:comment>element replacement of class attribute named <xsl:value-of select="$att"
          /></xsl:comment>
      ...
      <xsl:when test="key('CHANGEATT',$lookingAt)">
        <xsl:comment>element override of class attribute named <xsl:value-of select="$att"
          /></xsl:comment>
        <xsl:call-template name="mergeAttribute">
          <xsl:with-param name="New" select="key('CHANGEATT',$lookingAt)"/>
          <xsl:with-param name="Old" select="$orig"/>
