...
// ######################################################################
//
// Path: /usr/local/includes/php/roma/roma.php
//
// ######################################################################
...
// startroma.php
//
// furthermore there are a cupple of XQueries bundled with Roma
// 
...
class roma
  {
	...
	if ( $_REQUEST[ 'help' ] == 'switch' )
	...
	else
	  {
	      ...
	      catch( attributeExistsException $e )
		{
	      ...
	      $this->m_oRomaDom->customizeLanguage( $_REQUEST[ 'language' ] );
	      //notam
	      $oNotam = new notam();
	...
	$this->getListDom( 'http://' . roma_exist_server . '/xquery/elemsbymod.xq?module=' . $_REQUEST[ 'module' ], $oListDom );
     ...
     * in this function.
     * There are 5 different schema types available.
     * compiled.rng
     * rng
     * xsd
     * dtd
	...
	switch( $_REQUEST[ 'output' ] )
	  {
	  case 'compiled.rng':
	    $szError = $this->m_oRomaDom->createSchemaCompiledRNG( $szSchema, true );
	    break;
	  case 'rnc':
