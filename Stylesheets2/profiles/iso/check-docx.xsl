...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="w:p[w:pPr/w:pStyle and not(teidocx:is-supported-style(w:pPr/w:pStyle/@w:val))]" mode="document">
    ...
    <xsl:template match="w:p[w:pPr/w:pStyle and not(teidocx:is-supported-style(w:pPr/w:pStyle/@w:val))]" mode="comments">
        <xsl:variable name="number">
            <xsl:number select="." level="any"/>
        </xsl:variable>
        <xsl:variable name="id" select="$number"/>
        <xsl:call-template name="create-comment">
            ...
            <xsl:with-param name="text">Use of unsupported style detected: <xsl:value-of select="w:pPr/w:pStyle/@w:val"/></xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="w:p/w:r[w:rPr/w:rStyle and not(teidocx:is-supported-style(w:rPr/w:rStyle/@w:val))]" mode="document">
        <xsl:variable name="number">
            <xsl:number select="." level="any"/>
        </xsl:variable>
    ...
    <xsl:template match="w:p/w:r[w:rPr/w:rStyle and not(teidocx:is-supported-style(w:rPr/w:rStyle/@w:val))]" mode="comments">
        <xsl:variable name="number">
            <xsl:number select="." level="any"/>
        </xsl:variable>
        <xsl:variable name="id" select="$number+5000"/>
        <xsl:call-template name="create-comment">
            <xsl:with-param name="id" select="$id"/>
            <xsl:with-param name="text">Use of unsupported style detected: <xsl:value-of select="w:rPr/w:rStyle/@w:val"/></xsl:with-param>
        </xsl:call-template>
        <xsl:next-match/>
    </xsl:template>
