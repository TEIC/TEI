...
<xsl:message>Check whether styles used match styles defined: </xsl:message>
	...
	<xsl:message>Examine style <xsl:value-of select="$v"/></xsl:message>
	<xsl:for-each select="document('styles.xml',/)">
	  <xsl:choose>
	    <xsl:when test="count(key('S',$v))=0">
	      <xsl:message>ERROR: no entry for <xsl:value-of
	      select="$v"/></xsl:message>
	    </xsl:when>
	    <xsl:otherwise>
...
<xsl:message>Check whether rIds are listed in document rels: </xsl:message>
    <xsl:for-each select="key('RIDS',1)">
      ...
      <xsl:message>Examine <xsl:value-of select="$i"/></xsl:message>
