  ...
  <xsl:template match="tei:divGen[@type='actions']">
    ...
    <dl>
      <xsl:for-each select="/tei:TEI/tei:text//tei:note[@type='action']">
        <dt>
          <b>
            <xsl:number level="any" count="tei:note[@type='action']"/>
          </b>
        </dt>
        <dd>
          <xsl:apply-templates/>
...
<!-- 2. we are at or above splitting level, 
      ...
      <xsl:when test="$depth &lt;= $splitLevel and parent::tei:front
		      and $splitFrontmatter='true'">
	<xsl:call-template name="makeDivPage">
	  <xsl:with-param name="depth" select="$depth"/>
	</xsl:call-template>
      </xsl:when>

      <xsl:when test="$depth &lt;= $splitLevel and parent::tei:back and $splitBackmatter='true'">
	<xsl:call-template name="makeDivPage">
	  <xsl:with-param name="depth" select="$depth"/>
	</xsl:call-template>
      </xsl:when>
...
<xd:doc>
  ...
  <xsl:template name="includeJavascript">
	...
	function switchDiv(id) {
	  thisDiv = document.getElementById(curID); 	
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:apply-templates select="ancestor-or-self::tei:TEI/tei:text" mode="xpath">
...
<!-- we need to locate the first interesting object in the file, ie
      ...
      <xsl:otherwise>
		...
		<xsl:apply-templates select="ancestor-or-self::tei:TEI/tei:group/tei:text" mode="xpath">
		  <xsl:with-param name="xpath" select="$currentID"/>
		</xsl:apply-templates>
	      </xsl:when>
	      <xsl:otherwise>
...
<!-- header -->
	  ...
	  <a href="#rh-column" title="Go to main page content" class="skiplinks">Skip links</a></span>
        </div>
        <div id="hdr2">
...
<xsl:template name="bodyLayout">
    ...
    DO NOT EDIT (1)</xsl:comment>
      ...
      <head>
		...
		<xsl:for-each select="key('IDS',$currentID)">
	      ...
	      <xsl:otherwise>
		<xsl:apply-templates select="descendant-or-self::tei:TEI/tei:text" mode="xpath">
		  <xsl:with-param name="xpath" select="$currentID"/>
  ...
  <xsl:template name="previousLink">
    ...
    <xsl:choose>
      ...
      <xsl:when test="parent::tei:body/preceding-sibling::tei:back/tei:div[tei:head or $autoHead='true']">
        <xsl:apply-templates mode="generatePreviousLink" select="parent::tei:body/preceding-sibling::tei:back/tei:div[1]"/>
      </xsl:when>
      <xsl:when test="parent::tei:front/preceding-sibling::tei:body/tei:div[tei:head or $autoHead='true']">
...
<!-- back matter -->
  ...
  <xsl:template name="stdfooter">
	...
	<xsl:value-of select="$author"/>.
	  ...
	  on <xsl:call-template
	  name="whatsTheDate"/></xsl:comment></address>
    </div>
  </xsl:template>
