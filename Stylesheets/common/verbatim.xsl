...
<xsl:stylesheet xmlns:xd="http://www.pnp-software.com/XSLTdoc" xmlns:sch="http://www.ascc.net/xml/schematron" xmlns:m="http://www.w3.org/1998/Math/MathML" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:estr="http://exslt.org/strings" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dbk="http://docbook.org/ns/docbook" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:teix="http://www.tei-c.org/ns/Examples" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" exclude-result-prefixes="xlink xhtml dbk rng sch m tei teix xd atom">
  ...
  <xd:doc><xd:short>Reflow text</xd:short><xd:detail>[common] </xd:detail><xd:param name="indent">indentation to insert at start of line</xd:param><xd:param name="text">text to manage</xd:param>.
    <xd:param name="sofar">text already processed</xd:param></xd:doc>
...
  </xsl:text>
    ...
    <xsl:if test="following-sibling::token">
      ...
      <xsl:choose>
        <xsl:when test="contains(.,'--&gt;')">
          <xsl:apply-templates mode="normalline" select="following-sibling::token[1]"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:apply-templates mode="commentline" select="following-sibling::token[1]"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="token" mode="verbatimline">
    ...
    <xsl:if test="following-sibling::token">
      ...
      <xsl:apply-templates mode="verbatimline" select="following-sibling::token[1]"/>
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>
