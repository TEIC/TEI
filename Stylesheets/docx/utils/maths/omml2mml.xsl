...
<xsl:stylesheet 
	...
	<xsl:template match="m:phant">
		...
		<xsl:choose>
			...
			<xsl:when test="$fShow = 1">
				<xsl:element name="mpadded" xmlns="http://www.w3.org/1998/Math/MathML">
           ...
           use mphantom -->
			<xsl:when test="$fZeroWid=0 and $fZeroAsc=0 and $fZeroDesc=0">
				<xsl:element name="mphantom" xmlns="http://www.w3.org/1998/Math/MathML">
					<mrow>
						<xsl:apply-templates select="m:e" />
					</mrow>
				</xsl:element>
	...
	<xsl:template match="m:r">
		<xsl:variable name="sLowerCaseNor" select="translate(child::m:rPr[last()]/m:nor/@m:val, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 
		                                                                       'abcdefghijklmnopqrstuvwxyz')" />
		<xsl:variable name="sLowerCaseLit" select="translate(child::m:rPr[child::m:lit][last()]/@m:val, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 
		                                                                       'abcdefghijklmnopqrstuvwxyz')" />
