...
<xsl:template match="tei:/">
 <html> 
 <head>
 <title><xsl:apply-templates select="//tei:docTitle//text()"/></title>
 <link rel="stylesheet" type="text/css" href="teislides.css"/>
 </head>
  <frameset cols="20%,80%">
   <noframes>
  <h1><xsl:value-of select="/tei:TEI//tei:body//tei:docTitle"/></h1>
 ...
 <xsl:variable name="divname" select="concat($MasterFile,concat(generate-id()),'.html')"/>
  <a href="{$divname}" target="slides">
   ...
   <frame frameborder="0" src="{concat($MasterFile,generate-id(//tei:div1[1]))}.html" name="slides"/>
  </frameset>
 </html>
 <xsl:apply-templates select="TEI.2//tei:body//tei:div1"/>
<xsl:message>Create file 
   <xsl:value-of select="concat($MasterFile,'toc.html'"/>
</xsl:message>
 <xt:document method="html" doctype-public="-//tei:W3C//tei:DTD HTML 4.0 Transitional//tei:EN" href="{concat($MasterFile,'toc.html')}">
 <html> 
 <head>
 <title><xsl:apply-templates select="//tei:docTitle//text()"/></title>
 ...
 <body>
  <h1><xsl:value-of select="/TEI.2//tei:body//tei:docTitle"/></h1>
 <xsl:for-each select="//tei:body//tei:div1">
 <xsl:variable name="divname" select="concat($MasterFile,concat(generate-id()),'.html')"/>
  <a href="{$divname}" target="slides">
	<xsl:call-template name="header"/></a><br/>
 ...
 <xsl:call-template name="stdfooter">
	...
	select="/TEI.2/teiHeader//tei:revisionDesc//tei:date[1]"/>
 </xsl:call-template>
 </body>
 </html>
</xt:document>
...
<xsl:if test="preceding-sibling::div1">
  <a href="{concat($MasterFile,generate-id(preceding-sibling::div1[1]))}.html"><img src="left.gif" border="0" alt="PREVIOUS"/></a>
        ...
        <td>
  <a href="{concat($MasterFile,generate-id(//tei:div1[1]))}.html"><img src="up.gif" border="0" alt="FIRST"/></a>
  <a href="{concat($MasterFile,generate-id(//tei:div1[last()]))}.html"><img src="down.gif" border="0"  alt="LAST"/></a>
        </td>
        <td align="right">
...
<xsl:message>Create file 
<xsl:value-of select="concat($MasterFile,concat(generate-id()),'.html')"/>
</xsl:message>
  <xt:document method="html" doctype-public="-//tei:W3C//tei:DTD HTML 4.0 Transitional//tei:EN" href="{concat($MasterFile,concat(generate-id()),'.html')}">
	 <html> 
	 <head>
	 <title><xsl:apply-templates select="tei:head//text()"/></title>
         <link rel="stylesheet" type="text/css" href="teislides.css"/>
	 </head>
	 <body>
         <div  class="slide">
