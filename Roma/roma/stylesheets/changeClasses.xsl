...
<xsl:param name="host"/>
  ...
  <xsl:template name="generateList">
    <xsl:for-each select="//attClassList/attClass">
      <tr>
	<td><a><xsl:attribute name="href">http://<xsl:value-of select="$host"/>:8080/exist/TEI/Roma/xquery/class.xq?name=<xsl:value-of select="className"/></xsl:attribute><xsl:value-of select="className"/></a></td>
