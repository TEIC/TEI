...
<xsl:stylesheet exclude-result-prefixes="a t xd tei fo exsl rng edate xs"
	...
	xmlns:edate="http://exslt.org/dates-and-times" xmlns:exsl="http://exslt.org/common"
				...
				.=preceding-sibling::N[1]"/>
								<xsl:otherwise>
									...
									<xsl:choose>
										...
										<xsl:when test="self::N[1]='('"/>
										<xsl:when
											test="self::N[1]=')'and position() &lt; last()">
	...
	<xsl:template match="tei:attDef" mode="tangle">
		...
		<xsl:choose>
			...
			<xsl:when
				test="(tei:datatype/@maxOccurs &gt; 1) and tei:datatype/rng:data[@type='Name']">
				<xsl:text> NMTOKENS </xsl:text>
			</xsl:when>
