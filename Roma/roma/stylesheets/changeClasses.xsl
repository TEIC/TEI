...
<xsl:param name="host">http://localhost:8080/cocoon/Roma</xsl:param>
  ...
  <xsl:template name="generateList">
    <xsl:for-each select="//attClassList/attClass">
      <tr>
	<td><a><xsl:attribute name="href"><xsl:value-of select="$host"/>/Roma/xquery/class.xq?name=<xsl:value-of select="className"/></xsl:attribute><xsl:value-of select="className"/></a></td>
