<xsl:stylesheet
  ...
  <xsl:import
      href="http://www.tei-c.org/release/xml/tei/stylesheet/odds2/odd2html.xsl"/>
...
their encoding.</p>
	      ...
	      <ul>
		<xsl:for-each
		    select="/tei:TEI/tei:text/tei:body/tei:div">
...
select="ancestor-or-self::tei:TEI/tei:teiHeader/tei:fileDesc/tei:editionStmt/tei:edition"/>
    ...
    <xsl:if test="not($googleAnalytics='')">
      <script src="http://www.google-analytics.com/urchin.js"
	      type="text/javascript"><!-- load google analytics --></script>
      <script type="text/javascript">
...
<!-- German -->
  ...
  <xsl:template name="epubManifestHook">  
    ...
    <xsl:result-document method="text" href="COPYING.txt">
      <xsl:for-each	  select="tokenize(unparsed-text('COPYING.txt'),'\r?\n')">
	<xsl:value-of select="."/>
	<xsl:text>&#10;</xsl:text>
      </xsl:for-each>
    </xsl:result-document>
    ...
    <xsl:for-each select="key('ELEMENTDOCS',1)">
      ...
      <item xmlns="http://www.idpf.org/2007/opf"
	    media-type="application/xhtml+xml" id="ref-{$me}"
	    href="ref-{$me}.html" />
      <item xmlns="http://www.idpf.org/2007/opf"
	    media-type="application/xhtml+xml" id="examples-{$me}"
	    href="examples-{$me}.html" />
    </xsl:for-each>
  ...
  <xsl:template name="epubSpineHook">
    <xsl:for-each select="key('ELEMENTDOCS',1)">
      <xsl:variable name="me" select="@ident"/>
      <itemref xmlns="http://www.idpf.org/2007/opf" idref="ref-{$me}"
	       linear="no"/>
