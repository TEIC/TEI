...
(defun CUA-delete-rectangle ()
  (CUA-rect-operation nil nil t 1
    '(lambda (s e l r)
       (delete-region s e))))
...
(defun CUA-rect-aux-replace (width adjust keep replace format-fct &optional insert-fct)
...
A numeric prefix argument is used a new width for the filled rectangle."
  (interactive (list
		...
		(read-from-minibuffer "Enter text: "
				      nil nil nil nil)))
  (CUA-rect-aux-replace width t t t 
    'fill-region
    '(lambda () (insert text))))
...
A numeric prefix argument is used a new width for the filled rectangle."
  (interactive "p")
  (CUA-absorb-prefix-arg)
  (CUA-rect-aux-replace width t t t 'fill-region))
...
With prefix arg, replace rectangle with output from command."
  ...
  (CUA-rect-aux-replace -1 t t replace
    '(lambda (s e)
