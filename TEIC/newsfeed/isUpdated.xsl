...
-->
        ...
        <xsl:template name="main">
            ...
            <xsl:variable name="incomingUpdated" select="xs:dateTime(normalize-space(document($incoming)/atom:feed/atom:updated/text()))"/>
            <xsl:variable name="existingUpdated" select="xs:dateTime(normalize-space(document($existing)/atom:feed/atom:updated/text()))"/>
            <xsl:choose>
                <!-- If they are the same then don't update -->
                <xsl:when test="$incomingUpdated le $existingUpdated">NO</xsl:when>
                <!-- otherwise do update -->
