...
<xsl:stylesheet 
    exclude-result-prefixes="rng teix fo xi i a tei s sch xd xs html" 
    version="2.0"
  ...
  <xsl:template match="*" mode="literal">
    ...
    <xsl:for-each select="@*"><xsl:text> </xsl:text><xsl:value-of select="local-name(.)"
...
-->
        <xsl:choose>
          ...
          <xsl:when test="count(key('CLASSMEMBERS',$thisClass))&gt;0">
            <xsl:if test="$verbose='true'">
              <xsl:message> .... ... generate model <xsl:value-of select="$thisClass"/><xsl:value-of
                  select="$suffix"/>
              </xsl:message>
            </xsl:if>
            <define name="{$patternPrefixText}{$thisClass}{$suffix}"
  ...
  <xsl:template match="tei:elementSpec" mode="classmember">
    ...
    <ref name="{$elementPrefix}{@ident}"  xmlns="http://relaxng.org/ns/structure/1.0"/>
  </xsl:template>
        ...
        <Wrapper>
          ...
          <xsl:choose>
            <xsl:when test="tei:content/rng:notAllowed">
              <define name="{$elementPrefix}{@ident}"
                xmlns="http://relaxng.org/ns/structure/1.0">
                <notAllowed xmlns="http://relaxng.org/ns/structure/1.0"/>
              </define>
            </xsl:when>
...
-->
              ...
              <xsl:if test="$parameterize='true'">
                <define name="{$elementPrefix}{@ident}.content"
                  xmlns="http://relaxng.org/ns/structure/1.0">
                  <xsl:call-template name="defineContent"/>
                </define>
                <xsl:if test="not($Attributes='') or $TEIC='true'">
                  <define name="{$elementPrefix}{@ident}.localattributes"
                    xmlns="http://relaxng.org/ns/structure/1.0">
                    <xsl:call-template name="defineAttributes"/>
                  </define>
                </xsl:if>
...
-->
  ...
  <xsl:template name="defineContent">
    ...
    <xsl:choose>
      <xsl:when test="count($Contents/BLAH/*)=0">
        <rng:empty/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:for-each select="$Contents/BLAH">
          <xsl:copy-of select="*"/>
        </xsl:for-each>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="tei:macroSpec" mode="tangle">
    ...
    <xsl:variable name="entCount">
      <xsl:for-each select="$entCont/BLAH">
              ...
              <define name="{$macroPrefix}{@ident}"
                ...
                <xsl:choose>
                  <xsl:when test="starts-with(@ident,'type')">
                    <xsl:copy-of select="$entCont/BLAH/rng:*"/>
                  </xsl:when>
  ...
  <xsl:template name="makeSimpleAttribute">
    ...
    <rng:attribute name="{$name}">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$minOccurs='1' and number($maxOccurs) > 3">
          <xsl:call-template name="occursOutOfBounds">
            <xsl:with-param name="min">
              <xsl:value-of select="$minOccurs"/>
            </xsl:with-param>
        ...
        <xsl:when test="$minOccurs='2' and number($maxOccurs) > 2">
          <xsl:call-template name="occursOutOfBounds">
            <xsl:with-param name="min">
              <xsl:value-of select="$minOccurs"/>
            </xsl:with-param>
...
-->
 ...
 <xsl:template match="tei:constraintSpec"/>
  ...
  <xsl:template match="rng:ref" mode="forceRNG">
    <xsl:element name="ref" xmlns="http://relaxng.org/ns/structure/1.0">
